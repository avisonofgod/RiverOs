...
mipsel-linux-gnu-objcopy --set-start=0x80b71400 \
    --update-section .appended_dtb=$DTB vmlinux ../riveros-vmlinux || exit 1
mipsel-linux-gnu-strip ../riveros-vmlinux 2>/dev/null || true

echo "OK: ../riveros-vmlinux ($(stat -c%s ../riveros-vmlinux) bytes)"
