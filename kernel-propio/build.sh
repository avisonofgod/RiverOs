#!/bin/bash
# build.sh — kernel Risp-OS para MikroTik hEX (RB750Gr3)
...
echo "==> allnoconfig + config Risp-OS (minimal)"
...
scripts/config --set-str INITRAMFS_SOURCE "/root/proyectos/riveros/repo/rootfs"
...
cat vmlinux > ../riveros-vmlinux
ls -la ../riveros-vmlinux
echo "KERNEL OK: ../riveros-vmlinux ($(stat -c%s ../riveros-vmlinux) bytes)"
