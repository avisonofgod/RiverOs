...
Risp (backend ISP Rust) se monta encima opcionalmente (fase 2).
Todo por terminal; API Risp expuesta para frontend externo nativo.
...
- userspace: busybox (estático) + dropbear (estático) + Risp (Rust dinámico musl, fase 2)
- init: script shell mínimo estilo RiverOs (sin systemd, sin procd, sin OpenRC)
  ...
  - consola eth0 = 192.168.99.1/24 (whitelist Risp 8081)
  - dropbear :22 password (rbadmin2026, hash $5$riveros01)
  - hook Risp: si /etc/risp/zpot existe lo arranca (:80 portal, :8081 admin)
...
- risp: /root/rispm (cross-compile mipsel-unknown-linux-musl)

## Fases
1. [x] Toolchain mipsel: zig cc (linker musl) + gcc-mipsel-linux-gnu (kernel)
2. [x] Risp cross-compilado para mipsel (binario estatico)
3. [ ] Kernel 6.12.103 mt7621: config + DTS + build (vmlinux ELF)
4. [ ] Rootfs RAM: busybox + dropbear + Risp + init (netboot TFTP)
5. [ ] Flash NAND: driver NAND mt7621 + UBI + layout (persistencia)
6. [ ] Servicios: risp.init (arranque + respawn), red, firewall nftables

## Build
    ./build.sh          # descarga kernel, aplica config, cross-compila
    ./build-rootfs.sh   # ensambla initramfs/rootfs con busybox/dropbear/risp
